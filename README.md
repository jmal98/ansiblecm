...
```bash
docker build --tag ansiblecm:2.5.6 .
```
...
```bash
docker run -it --rm -v <absolute path to playbook>:/tmp/playbook:Z jmal98/ansiblecm:2.5.6 <playbook arguments>
```
...
```bash
docker run -it --rm -v $PWD:/tmp/playbook:Z jmal98/ansiblecm:2.5.6 site.yml -i inventory/hosts
```
...
```bash
docker run -it --rm --entrypoint ansible jmal98/ansiblecm:2.5.6 web -m setup -i inventory
```
