...
```bash
docker build --tag ansiblecm:2.8.5 .
```
...
```bash
docker run -it --rm -v <absolute path to playbook>:/tmp/playbook:Z jmal98/ansiblecm:2.8.5 <playbook arguments>
```
...
```bash
docker run -it --rm -v $PWD:/tmp/playbook:Z jmal98/ansiblecm:2.8.5 site.yml -i inventory/hosts
```
...
```bash
docker run -it --rm --entrypoint ansible jmal98/ansiblecm:2.8.5 web -m setup -i inventory
```
