...
```bash
docker build --tag ansiblecm:4.4.0 .
```
...
```bash
docker run -it --rm -v <absolute path to playbook>:/tmp/playbook:Z jmal98/ansiblecm:4.4.0 <playbook arguments>
```
...
```bash
docker run -it --rm -v $PWD:/tmp/playbook:Z jmal98/ansiblecm:4.4.0 site.yml -i inventory/hosts
```
...
```bash
docker run -it --rm --entrypoint ansible jmal98/ansiblecm:4.4.0 web -m setup -i inventory
```
